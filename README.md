...
### Security
Multi-user support is under active development; this is an early piece of it. On first launch, a setup screen walks you through picking a security mode — **No login**, **Password** (a single shared password, set right there), or **Sign in with…** (OIDC/SSO, greyed out here since it needs the env var below). Setting `AUTH_METHOD` yourself locks that screen to your choice instead of leaving it pickable (a `singlepass` password is still entered there either way, since a password itself has no environment-variable equivalent).

> [!NOTE]
> **This setting doesn't gate access yet.** It only drives the first-launch setup screen so far — see the caution at the top of this README, which still fully applies regardless of what you set here. Real request-level enforcement (sessions, per-user permissions) is planned but not yet built.
