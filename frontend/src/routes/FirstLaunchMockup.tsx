...
type AuthChoice = 'none' | 'singlepass'

const DOCKER_FIXTURE_PATH = '/data'
const NATIVE_FIXTURE_PATHS = [
...
// BookUploadAboutStep.tsx). Defined once here rather than copy-pasted per
// step, since this whole flow lives in a single mockup file.
...
function FolderStep({
  ...
  const [nativePathIndex, setNativePathIndex] = useState(0)
  const nativePath = NATIVE_FIXTURE_PATHS[nativePathIndex]
        ...
        {mode === 'docker' ? (
          <>
            <div className="flex items-center gap-3 rounded-md border border-border bg-paper-sunken px-4 py-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate font-mono text-sm text-ink">{DOCKER_FIXTURE_PATH}</p>
          ...
          <>
            <div className="flex items-center gap-2">
              ...
              <button
                type="button"
                onClick={() => setNativePathIndex((i) => (i + 1) % NATIVE_FIXTURE_PATHS.length)}
                className="flex shrink-0 cursor-pointer items-center gap-1.5 rounded-md border border-border bg-paper-raised px-3 py-2 text-sm text-ink hover:border-accent"
              >
...
function SecurityStep({
  ...
  mode: RuntimeMode
  onBack: () => void
  onFinish: (choice: AuthChoice) => void
}) {
  ...
  const canFinish = choice === 'none' || (choice === 'singlepass' && passwordValid)
...
export function FirstLaunchMockup() {
  useMockupTitle('First-Time Launch Flow')
  const [step, setStep] = useState<Step>('welcome')
  const [mode, setMode] = useState<RuntimeMode>('docker')
  const [finishedChoice, setFinishedChoice] = useState<AuthChoice>('none')

  return (
    <div className="flex min-h-dvh flex-col items-center justify-center bg-paper p-6">
      <RuntimeModeToggle mode={mode} onChange={setMode} />
