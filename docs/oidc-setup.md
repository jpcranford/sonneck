...
Changing `AUTH_METHOD` on an install that's already been through first-launch setup currently just takes effect silently on the next restart — there's no guided walk-through yet for that transition (for example, choosing which account should survive if you're moving away from OIDC and have more than one account). That's still ahead.
