...
// composer/arranger are deliberately absent from this INSERT (and from
// GetPieceByID/UpdatePiece below) — the composer/arranger overhaul
// (migration 00020) moved them to ordered join tables (ComposerIDs/
// ArrangerIDs, set via SetPieceComposers/SetPieceArrangers). The old
// `pieces.composer`/`pieces.arranger` TEXT columns still exist in the
// schema but are unused by application code from this point on; see that
// migration's own comment for why they aren't dropped yet.
func CreatePiece(ctx context.Context, q Queryer, p *models.Piece) (int64, error) {
	res, err := q.ExecContext(ctx, `
		...
		p.CopyrightHolder, p.CopyrightSlug, p.CopyrightStatus, p.CopyrightRenewed,
	)
	if err != nil {
		return 0, err
	}
...
func GetPieceByID(ctx context.Context, q Queryer, id int64) (*models.Piece, error) {
	...
	err := q.QueryRowContext(ctx, `
		...
		&p.Duration, &p.BPM, &p.MeasureCount, &p.BeatsPerMeasure,
		&p.FilePath, &p.FileHash, &p.PageCount, &p.ThumbnailPage, &p.CopyrightYear,
		&p.CopyrightHolder, &p.CopyrightSlug, &p.CopyrightStatus, &p.CopyrightRenewed, &p.CreatedAt, &p.UpdatedAt,
	)
	...
	composerIDs, err := getPieceComposerIDs(ctx, q, p.ID)
	if err != nil {
		return nil, err
	}
	p.ComposerIDs = composerIDs

	arrangerIDs, err := getPieceArrangerIDs(ctx, q, p.ID)
	if err != nil {
		return nil, err
	}
...
// UserTagIDs/ComposerIDs/ArrangerIDs) are not written here — use
// SetPieceKeys/SetPieceInstruments/SetPieceUserTags/SetPieceComposers/
// SetPieceArrangers.
